...
exports.buildSpinePanel = buildSpinePanel;
const template_utils_1 = require("@m0saic/template-utils");
const common_1 = require("./common");
function verticalTitlePieces(title, geometry, direction) {
    ...
    if (!chars.length)
        return [];
    const band = (0, common_1.sliceRect)(geometry.safe, 0.05, 0.16, 0.9, 0.62);
...
function buildSpinePanel(args) {
    const { props, variant, validation, geometry, assets } = args;
    const pieces = [];
    const children = {};
    if (props.spineArt)
        pieces.push((0, common_1.mediaPiece)(geometry.paint, assets.media(props.spineArt, "spine art", "cover"), 0));
    else
        pieces.push((0, common_1.solidPiece)(geometry.paint, props.backColor ?? "#10141d", "spine surface", 0));
    if (props.studioLogoArt) {
        pieces.push((0, common_1.mediaPiece)((0, common_1.sliceRect)(geometry.safe, 0.08, 0.01, 0.84, 0.1), assets.media(props.studioLogoArt, "spine studio logo"), 3));
    }
    const titlePieces = verticalTitlePieces(variant.title, geometry, variant.profile.spineTextDirection);
    if (titlePieces.length) {
        ...
        const grownH = (0, template_utils_1.ceilToSmooth)(rawBounds.h);
        const titleBounds = {
            ...rawBounds,
    ...
    const catalogRect = (0, common_1.sliceRect)(geometry.safe, 0.02, 0.8, 0.96, 0.08);
    pieces.push({
        rect: { ...catalogRect, importance: 3 },
        source: (0, common_1.textSource)({ text: variant.catalogNumber, fontSize: Math.max(6, geometry.safe.w * 0.25), color: "#ffffff", label: "spine catalog number", hAlign: "center" }),
    });
    ...
    if (rating && (validation.ratingBadgeArt || props.usePlaceholderBadges)) {
        const size = Math.max(10, Math.min(Math.round(rating.minSizeMm * (0, common_1.pxPerMm)(geometry.dpi)), Math.round(geometry.trim.w * 0.82)));
        const rect = (0, common_1.cornerRect)(geometry.trim, rating.corner, size, Math.max(1, Math.round((0, common_1.pxPerMm)(geometry.dpi))));
        if (validation.ratingBadgeArt)
            pieces.push((0, common_1.mediaPiece)(rect, assets.media(validation.ratingBadgeArt, "spine rating badge"), 4));
        else {
            pieces.push((0, common_1.solidPiece)(rect, "#20242c", "spine rating placeholder", 4));
            pieces.push({ rect: { ...rect, importance: 5 }, source: (0, common_1.textSource)({ text: variant.profile.ratingSystem, fontSize: Math.max(6, size * 0.16), color: "#ffffff", label: "spine rating placeholder text", hAlign: "center" }) });
        }
    }
