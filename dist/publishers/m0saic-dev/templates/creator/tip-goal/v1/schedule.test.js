...
describe("auto generator", () => {
    it("is deterministic per seed and differs across seeds", () => {
        const a = (0, schedule_1.generateAutoTips)({ seed: 7 }, 0, 100, 15, []);
        const b = (0, schedule_1.generateAutoTips)({ seed: 7 }, 0, 100, 15, []);
        const c = (0, schedule_1.generateAutoTips)({ seed: 8 }, 0, 100, 15, []);
        expect(a).toEqual(b);
        expect(JSON.stringify(a)).not.toBe(JSON.stringify(c));
    });
    it("sums exactly to goal - start with integer tips >= 1", () => {
        const tips = (0, schedule_1.generateAutoTips)({ seed: 3, tipCount: 12 }, 10, 100, 15, []);
    ...
    it("stays inside [startDelay, finishFrac*duration], ascending", () => {
        const tips = (0, schedule_1.generateAutoTips)({ seed: 2, startDelaySec: 2, finishFrac: 0.8 }, 0, 100, 20, []);
        for (let i = 0; i < tips.length; i++) {
            expect(tips[i].atSec).toBeGreaterThanOrEqual(2);
            expect(tips[i].atSec).toBeLessThanOrEqual(16 + 1e-9);
            if (i > 0)
                expect(tips[i].atSec).toBeGreaterThanOrEqual(tips[i - 1].atSec);
        }
    ...
    it("shrinks the tip count when the pot is smaller than the count", () => {
        const tips = (0, schedule_1.generateAutoTips)({ seed: 1, tipCount: 12 }, 0, 5, 15, []);
        expect(tips.length).toBeLessThanOrEqual(5);
        expect(tips.reduce((s, t) => s + t.amount, 0)).toBe(5);
    });
    it("start >= goal → no tips + a warning", () => {
        const warnings = [];
        const tips = (0, schedule_1.generateAutoTips)({}, 100, 100, 15, warnings);
        expect(tips).toEqual([]);
...
describe("tips → anchors", () => {
    it("holds, then rises over riseSec on each tip", () => {
        ...
        expect((0, schedule_1.evalAnchors)(out.anchors, 0)).toBe(0);
        expect((0, schedule_1.evalAnchors)(out.anchors, 4.99)).toBe(0);
        const mid = (0, schedule_1.evalAnchors)(out.anchors, 5.3);
        expect(mid).toBeGreaterThan(0);
        expect(mid).toBeLessThan(20);
        expect((0, schedule_1.evalAnchors)(out.anchors, 7)).toBe(20);
        expect((0, schedule_1.evalAnchors)(out.anchors, 14)).toBe(50);
    });
    it("crowded tips shorten the rise instead of overlapping", () => {
        const out = (0, schedule_1.resolveSchedule)({ ...baseInput, tips: [{ atSec: 5, amount: 10 }, { atSec: 5.2, amount: 10 }] }, 15);
        if (!out.ok)
            throw new Error(out.message);
    ...
    it("refunds clamp the running total at 0", () => {
        const out = (0, schedule_1.resolveSchedule)({ ...baseInput, tips: [{ atSec: 2, amount: -50 }] }, 15);
        if (!out.ok)
            throw new Error(out.message);
        expect(out.finalAmount).toBe(0);
    });
    it("over the tip budget → TG_TOO_DENSE", () => {
        const tips = Array.from({ length: schedule_1.MAX_TIPS + 1 }, (_, i) => ({
            atSec: i * 0.05,
            amount: 1,
        }));
        const out = (0, schedule_1.resolveSchedule)({ ...baseInput, tips }, 60);
        expect(out.ok).toBe(false);
        if (!out.ok)
            expect(out.code).toBe("TG_TOO_DENSE");
    });
});
describe("schedule (absolute keys)", () => {
    it("wins over tips and flashes on rising keys only", () => {
        const out = (0, schedule_1.resolveSchedule)({
            ...baseInput,
    ...
    it("an empty schedule array falls through to auto", () => {
        const out = (0, schedule_1.resolveSchedule)({ ...baseInput, schedule: [] }, 15);
        if (!out.ok)
            throw new Error(out.message);
        expect(out.mode).toBe("auto");
    });
    it("a non-array schedule → TG_SCHEDULE_PARSE", () => {
        const out = (0, schedule_1.resolveSchedule)({ ...baseInput, schedule: "x" }, 15);
        expect(out.ok).toBe(false);
        if (!out.ok)
            expect(out.code).toBe("TG_SCHEDULE_PARSE");
    });
});
describe("expression builders", () => {
    const anchors = [
        { t: 0, v: 0 },
        { t: 5, v: 0, ease: "easeOut" },
        { t: 5.6, v: 120 },
    ];
    it("fill xExpr slides by the goal-clamped fraction", () => {
        const expr = (0, schedule_1.buildFillXExpr)(anchors, 100);
    ...
    it("counter expr is drawtext-safe (escaped commas, eif wrapper)", () => {
        const expr = (0, schedule_1.buildCounterTextExpr)(anchors, "$", " / $100");
    ...
    it("literal prefix/suffix control chars are escaped", () => {
        const expr = (0, schedule_1.buildCounterTextExpr)(anchors, "100%", "{x}");
        expect(expr).toContain("100\\%");
        expect(expr).toContain("\\{x\\}");
    });
    it("flash gate is a rebalanced between-union with a window", () => {
        const gate = (0, schedule_1.buildTipFlashGate)([2, 8.5]);
        if (!gate)
        ...
        expect((0, schedule_1.buildTipFlashGate)([])).toBeUndefined();
    });
});
