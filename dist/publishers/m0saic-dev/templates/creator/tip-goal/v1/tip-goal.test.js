...
const tip_goal_1 = require("./tip-goal");
...
async function render(props, ctx = makeCtx()) {
    return (await tip_goal_1.TipGoalV1.render(props, ctx));
}
...
describe("zero-props render (the demo contract)", () => {
    it("succeeds with the full document tree", async () => {
        const doc = await render({ ...tip_goal_1.TipGoalV1.defaultProps });
...
describe("error paths (TG_* codes)", () => {
    const cases = [
        ...
        ["tips not an array", { tips: "x" }, "TG_TIPS_PARSE"],
        ["tips all invalid", { tips: [{ atSec: "a", amount: null }] }, "TG_TIPS_PARSE"],
        ["schedule not an array", { schedule: "x" }, "TG_SCHEDULE_PARSE"],
        ...
        ["geometry not an object", { geometry: [1] }, "TG_GEOMETRY_PARSE"],
        [
            "geometry bad rect",
...
describe("determinism", () => {
    ...
    it("duration scales the auto schedule (window follows the clip)", async () => {
        ...
        const lastWindow = (doc) => doc.children.tg_bar.sources[2].overlay.window.endSec;
