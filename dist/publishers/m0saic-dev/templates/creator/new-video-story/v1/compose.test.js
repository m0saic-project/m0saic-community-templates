...
const H = 1920;
function cfg(props = {}) {
    const out = (0, resolve_1.resolveNewVideoStory)(props);
    if (!out.ok)
        throw new Error(out.message);
    return out.cfg;
}
function build(props = {}, w = W, h = H, kinds = {}) {
    const out = (0, compose_1.buildNewVideoStoryDoc)(cfg(props), w, h, 30, 8000, (p) => kinds[p] ?? (/\.mp4$/i.test(p) ? "video" : "image"));
    if (!out.ok)
        throw new Error(`${out.code}: ${out.message}`);
    return out;
}
const labelsOf = (doc) => doc.sources.map((s) => s.editor?.label);
const find = (doc, label) => doc.sources.find((s) => s.editor?.label === label);
/** Painted rect of a source: its quantized frame through the engine's inset floor math. */
function painted(doc, source) {
...
describe("buildNewVideoStoryDoc — the default story", () => {
    ...
    it("binds what it shows: the screenshot rect, the copy, the accent", () => {
        ...
        for (const s of doc.sources.filter((x) => x.editor?.label === "headline" || x.editor?.label === "headline-top")) {
    ...
    it("every piece paints on its exact intent rect (inset recovery, zero drift)", () => {
        out.expectations.forEach((exp, i) => {
    ...
    it("the sticker is white glyphs on a black stroke on a white halo, with a shared outline", () => {
        const [halo, stroke, fill] = doc.sources.filter((s) => s.editor?.label === "headline");
    ...
    it("motion: every piece animates from its own cue, in pixel literals", () => {
        ...
        const arrows = doc.sources.filter((s) => s.editor?.label === "arrow");
        expect(arrows.map((a) => a.overlay.startAtSec)).toEqual([1.15, 1.3, 1.45]);
        for (const a of arrows)
            expect(a.overlay.yExpr).toContain("cos(2*PI*");
...
describe("buildNewVideoStoryDoc — knobs", () => {
    it("animate off: no overlay anywhere; stats say static", () => {
        ...
        for (const s of out.doc.sources)
    ...
    it("a URL screenshot becomes a url asset", () => {
        const doc = build({ media: "https://cdn.example/shot.png" }).doc;
    ...
    it("badge image replaces the drawn glyph; badge none drops the slot; accent recolours", () => {
        const img = build({ badgeImage: "yt.png" }).doc;
        ...
        const none = build({ badge: "none" }).doc;
    ...
    it("platform presets change copy, glyph and accent; the stage colour is a knob", () => {
        const tw = build({ platform: "twitch", backgroundColor: "#111111" }).doc;
