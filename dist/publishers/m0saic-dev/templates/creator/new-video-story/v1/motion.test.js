 ...
 */
function ev(expr, t) {
    const fn = new Function("t", "min", "max", "pow", "cos", "gte", "PI", `return (${expr});`);
    return fn(t, Math.min, Math.max, Math.pow, Math.cos, (a, b) => (a >= b ? 1 : 0), Math.PI);
}
function onlyKnownNames(expr) {
...
describe("timeline", () => {
    it("runs in order and settles before the poster time", () => {
        ...
        expect(T.bobAt).toBeCloseTo(T.arrowsAt[2] + motion_1.ARROW_RISE_SEC, 5);
        // Poster: after the pill settles, on a bob rest point.
        expect(motion_1.SETTLED_AT_SEC).toBeGreaterThanOrEqual(T.pillAt + motion_1.POP_SEC);
        expect(motion_1.SETTLED_AT_SEC).toBeLessThan(3);
        expect(((motion_1.SETTLED_AT_SEC - T.bobAt) / motion_1.BOB_PERIOD_SEC) % 1).toBeCloseTo(0, 6);
    });
});
describe("easeOutBackExpr", () => {
    it("0 at the start, overshoots past 1, settles at exactly 1", () => {
        const e = (0, motion_1.easeOutBackExpr)(1, 0.5);
...
describe("entrances (pixel literals, absolute time)", () => {
    it("slideInX: off by the full distance before start, at rest after", () => {
        const m = (0, motion_1.slideInX)(0.25, 0.5, -799);
    ...
    it("dropIn: starts above by the distance, dips below rest on the bounce, settles at 0", () => {
        const m = (0, motion_1.dropIn)(0.55, 0.55, -300);
    ...
    it("riseIn: alpha 0→1 while drifting up from driftPx", () => {
        const m = (0, motion_1.riseIn)(0.85, 0.45, 30);
    ...
    it("popIn: a fast fade with an overshooting rise from below", () => {
        const m = (0, motion_1.popIn)(1.65, 0.5, 71);
...
describe("arrow bob", () => {
    it("bobTerm: 0 before and at the start, peaks at half a period, back to 0 at a period", () => {
        const b = (0, motion_1.bobTerm)(1.75, 23);
        expect(ev(b, 0)).toBe(0);
        expect(ev(b, 1.75)).toBeCloseTo(0, 6);
        expect(ev(b, 1.75 + motion_1.BOB_PERIOD_SEC / 2)).toBeCloseTo(23, 6);
        expect(ev(b, 1.75 + motion_1.BOB_PERIOD_SEC)).toBeCloseTo(0, 6);
        expect(ev(b, 1.75 + 7 * motion_1.BOB_PERIOD_SEC + motion_1.BOB_PERIOD_SEC / 2)).toBeCloseTo(23, 6);
        expect(onlyKnownNames(b)).toBe(true);
    });
    it("arrowMotion: the rise then the bob, continuous at the hand-off", () => {
        const m = (0, motion_1.arrowMotion)(1.15, 35, 1.75, 23);
