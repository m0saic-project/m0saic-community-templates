...
exports.FACECAM_CORNERS = void 0;
exports.selectAspect = selectAspect;
exports.computeRegions = computeRegions;
exports.fitCapsFontPx = fitCapsFontPx;
exports.computeCardGeometry = computeCardGeometry;
exports.spotlightRect = spotlightRect;
exports.ringRects = ringRects;
function selectAspect(width, height) {
    if (width <= 0 || height <= 0)
        return "DESKTOP";
...
exports.FACECAM_CORNERS = ["top-left", "top-right", "bottom-left", "bottom-right"];
 ...
 */
function computeRegions(W, H, hasFacecam, aspect, stage = { safe: { x: 0, y: 0, w: W, h: H } }, pip = { corner: "top-left", sizeFrac: 0.34 }, 
...
facecamOverride) {
    ...
    const clearRail = (r) => {
        ...
        if (!rail)
            return r;
        const overlaps = rail.x < r.x + r.w && rail.x + rail.w > r.x && rail.y < r.y + r.h && rail.y + rail.h > r.y;
    ...
    const camW = aspect === "DESKTOP"
        ? Math.min(idealCamW, Math.round(W * 0.4))
        : Math.round(W * 0.38);
    const left = Math.max(safe.x, camW);
    const card = inset(clearRail({ x: left, y: safe.y, w: Math.max(1, safe.x + safe.w - left), h: safe.h }), margin);
...
function fitCapsFontPx(text, rect, heightCap, em) {
...
function computeCardGeometry(card, weekRows, mastheadText, 
...
paperRect) {
    ...
    if (listRowCount > 0) {
        ...
        if (listRowCount * listRowH > maxListH)
            listRowH = Math.max(14, Math.floor(maxListH / listRowCount));
    ...
    const colX = Array.from({ length: 8 }, (_, k) => Math.round(innerX + (k * innerW) / 7));
    // Row lattice: header boundary, then equal week rows.
    const rowY = [tableTop, tableTop + headerH];
    ...
    const dayCells = Array.from({ length: weekRows }, (_, r) => Array.from({ length: 7 }, (_, c) => cellAt(c, r + 1)));
    const table = {
        x: innerX,
        y: tableTop,
        w: innerW,
    ...
    const listRows = Array.from({ length: listRowCount }, (_, i) => ({
        x: innerX,
 ...
 */
function spotlightRect(table, sizeFrac) {
...
function ringRects(r, thickness) {
