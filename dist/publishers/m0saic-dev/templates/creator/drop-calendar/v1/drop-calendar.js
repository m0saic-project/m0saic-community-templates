 ...
 */
function resolveDurationMs(ctx, facecamProbe, reelTotalMs) {
    const natural = typeof reelTotalMs === "number" && Number.isFinite(reelTotalMs) && reelTotalMs > 0
        ? reelTotalMs
...
function calendarError(ctx, code, message) {
...
exports.DropCalendarV1 = (0, template_utils_1.defineMosaicTemplate)({
    ...
    propsSchema: types_2.DropCalendarPropsSchema,
    ...
    resolveOutputHints: (props) => {
        const canvas = (0, platforms_1.resolveCreatorPlatform)(props.platform).canvas;
    ...
    async render(props, ctx) {
        ...
        if (facecamProbe?.kind === "image" || facecamProbe?.kind === "audio") {
            return calendarError(ctx, "DC_FACECAM_KIND", `The facecam must be a video — ${JSON.stringify(facecamPath)} probed as ${facecamProbe.kind}.`);
        }
        ...
        // through it.
        const facecamReel = facecamPath !== ""
            ? (0, facecamReel_1.resolveFacecamReel)(props.facecamClips, {
        ...
        const resolved = (0, resolve_1.resolveDropCalendar)(props, durationMs / 1000, facecamReel);
        if (!resolved.ok)
            return calendarError(ctx, resolved.code, resolved.message);
        const built = (0, compose_1.buildDropCalendarDoc)(resolved.cfg, W, H, fps, durationMs, (path) => (0, resolve_1.classifyMedia)(path, probeMedia(ctx, path)?.kind));
        if (!built.ok)
            return calendarError(ctx, built.code, built.message);
