...
const reelOf = (raw, opts = {}) => {
    const out = (0, facecamReel_1.resolveFacecamReel)(raw, opts);
    if (!out.reel)
        throw new Error(`expected a reel, got warnings: ${out.warnings.join(" | ")}`);
    return out.reel;
};
describe("no reel (play the whole facecam)", () => {
    it("returns nothing for absent / empty clips, without warning", () => {
        for (const raw of [undefined, null, []]) {
            const out = (0, facecamReel_1.resolveFacecamReel)(raw);
            expect(out.reel).toBeUndefined();
            expect(out.warnings).toEqual([]);
        }
    });
    it("degrades (with a warning) on an unparsable payload", () => {
        const out = (0, facecamReel_1.resolveFacecamReel)("{not json");
        expect(out.reel).toBeUndefined();
        expect(out.warnings[0]).toContain("whole facecam");
    });
    it("degrades when every clip is unusable", () => {
        const out = (0, facecamReel_1.resolveFacecamReel)([{ startMs: 9000, endMs: 12000 }], {
...
describe("one clip", () => {
    ...
    it("clamps to the probed source length", () => {
        const out = (0, facecamReel_1.resolveFacecamReel)([{ startMs: 1000, endMs: 99000 }], {
...
describe("multi-clip reel", () => {
    ...
    it("keeps the picked order and skips only the bad entries", () => {
        const out = (0, facecamReel_1.resolveFacecamReel)([
    ...
    it("caps the clip count", () => {
        const many = Array.from({ length: facecamReel_1.MAX_FACECAM_CLIPS + 3 }, (_, i) => ({
            startMs: i * 1000,
            endMs: i * 1000 + 500,
        }));
        const out = (0, facecamReel_1.resolveFacecamReel)(many, { transitionStyle: "cut" });
        expect(out.reel?.clips).toHaveLength(facecamReel_1.MAX_FACECAM_CLIPS);
...
describe("transition normalization", () => {
    it("takes any engine xfade kernel, falls back to fade", () => {
        expect((0, facecamReel_1.normalizeTransition)("wipeleft")).toBe("wipeleft");
        expect((0, facecamReel_1.normalizeTransition)("cut")).toBe("cut");
        expect((0, facecamReel_1.normalizeTransition)("custom")).toBe("fade"); // deliberately unsupported
        expect((0, facecamReel_1.normalizeTransition)(undefined)).toBe("fade");
    });
    it("clamps the overlap length", () => {
        expect((0, facecamReel_1.normalizeTransitionMs)(undefined)).toBe(400);
        expect((0, facecamReel_1.normalizeTransitionMs)(0)).toBe(50);
        expect((0, facecamReel_1.normalizeTransitionMs)(99)).toBe(3000);
        expect((0, facecamReel_1.normalizeTransitionMs)(1.25)).toBe(1250);
    });
});
describe("cue mapping (source time → the cut timeline)", () => {
    ...
    it("carries a cue inside a clip onto its reel position", () => {
        expect((0, facecamReel_1.mapFacecamWindow)(reel, 12000)).toEqual({ startMs: 2000 });
        // Clip 2 opens at 5000 − 500 (the overlap) = 4500.
        expect((0, facecamReel_1.mapFacecamWindow)(reel, 61000)).toEqual({ startMs: 5500 });
    });
    it("maps a windowed cue and clamps an end past the cut", () => {
        expect((0, facecamReel_1.mapFacecamWindow)(reel, 11000, 13000)).toEqual({ startMs: 1000, endMs: 3000 });
        expect((0, facecamReel_1.mapFacecamWindow)(reel, 14000, 20000)).toEqual({ startMs: 4000, endMs: 5000 });
    });
    it("drops a cue that lands in footage the user cut out", () => {
        expect((0, facecamReel_1.mapFacecamWindow)(reel, 30000)).toBeUndefined();
        expect((0, facecamReel_1.mapFacecamWindow)(reel, 15000)).toBeUndefined(); // end is exclusive
    });
});
