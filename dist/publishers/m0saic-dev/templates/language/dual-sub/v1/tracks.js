...
exports.scoreDialogueLikeness = scoreDialogueLikeness;
exports.selectLearnTrack = selectLearnTrack;
 ...
 */
function scoreDialogueLikeness(track) {
    ...
    const events = [];
    for (const c of cues)
        events.push([c.startMs, 1], [c.endMs, -1]);
 ...
 */
function selectLearnTrack(tracks, sel) {
    ...
    const best = (candidates) => {
        ...
        for (const track of candidates) {
            ...
            if (!top || score > top.score)
                top = { track, rule: "dialogue-score", score };
        }
        return top;
    };
    if (sel.languageCode) {
        const matches = tracks.filter((t) => t.stream.language && (0, platform_1.matchesLanguageCode)(t.stream.language, sel.languageCode));
        const pick = best(matches);
        if (pick)
            return { ...pick, rule: "language-match" };
        return null;
    }
    return best(tracks);
}
