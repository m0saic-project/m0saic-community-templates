...
describe("computeLayout — bar geometry", () => {
    const layout = (0, layout_1.computeLayout)(DEFAULT_INPUT);
    ...
    it("places the icon square in the left pad zone, vertically centered", () => {
        const iconSize = Math.round(layout_1.ICON_FRAC * layout.bar.h);
        expect(layout.icon).toEqual({
            x: layout.bar.x + Math.round(layout_1.PAD_X_FRAC * layout.bar.h),
    ...
    it("omits the icon zone when hidden — the label starts at the pad", () => {
        const layout2 = (0, layout_1.computeLayout)({ ...DEFAULT_INPUT, showIcon: false });
        expect(layout2.icon).toBeNull();
        expect(layout2.labelX).toBe(layout2.bar.x + layout2.padXPx);
    });
});
describe('computeLayout — frame "fill"', () => {
    const layout = (0, layout_1.computeLayout)({ ...DEFAULT_INPUT, frame: "fill" });
    ...
    it("scales the zones off the full canvas height", () => {
        expect(layout.icon?.w).toBe(Math.round(layout_1.ICON_FRAC * 400));
        expect(layout.padXPx).toBe(Math.round(layout_1.PAD_X_FRAC * 400));
        expect(layout.underlineY + layout_1.UNDERLINE_H_PX).toBeLessThanOrEqual(400);
    });
    it("still clamps the corner radius to half the box height", () => {
        const pill = (0, layout_1.computeLayout)({ ...DEFAULT_INPUT, frame: "fill", cornerRadiusPx: 999 });
...
describe("computeLayout — shrink-to-fit fontSize (D9)", () => {
    it("converges to a size whose required width fits inside the slack margin", () => {
        const layout = (0, layout_1.computeLayout)(DEFAULT_INPUT);
        expect(layout.fontSize).toBeLessThanOrEqual(layout_1.FONT_START_FRAC * layout.bar.h);
        expect(layout.fontSize).toBeGreaterThanOrEqual(layout_1.MIN_FONT_PX);
        const iconSize = Math.round(layout_1.ICON_FRAC * layout.bar.h);
        const iconGap = layout.labelX - (layout.bar.x + layout.padXPx + iconSize);
        const required = 2 * layout.padXPx + iconSize + iconGap + layout.labelWidthPx + layout.maxWordWidthPx;
        expect(required).toBeLessThanOrEqual(layout.bar.w * (1 - layout_1.FIT_SLACK));
    });
    it("shrinks further for longer content", () => {
        const short = (0, layout_1.computeLayout)({ ...DEFAULT_INPUT, words: ["Go"] });
        const long = (0, layout_1.computeLayout)({ ...DEFAULT_INPUT, words: ["Recommendations galore"] });
        expect(long.fontSize).toBeLessThanOrEqual(short.fontSize);
    });
    it("throws fail-fast when even the font floor overflows", () => {
        expect(() => (0, layout_1.computeLayout)({
            ...DEFAULT_INPUT,
            canvasW: 400,
            canvasH: 100,
...
describe("computeLayout — label and word origin", () => {
    it("appends the separator space and starts words right after it", () => {
        ...
        expect(layout.labelWidthPx).toBeCloseTo((0, template_utils_1.measureText)(layout.labelText, { fontSize: layout.fontSize }).width, 6);
    ...
    it("hides an empty label without leaving a gap", () => {
        const layout = (0, layout_1.computeLayout)({ ...DEFAULT_INPUT, label: "" });
...
describe("computeLayout — vertical group", () => {
    ...
    it("places the underline below the descent with the fixed gap", () => {
        expect(layout.underlineY).toBe(Math.round(layout.baselineY + layout.descent + layout_1.UNDERLINE_GAP_PX));
        expect(layout.underlineHeightPx).toBe(layout_1.UNDERLINE_H_PX);
        expect(layout.underlineY + layout_1.UNDERLINE_H_PX).toBeLessThanOrEqual(layout.bar.y + layout.bar.h);
    });
...
describe("charAdvances", () => {
    it("returns L+1 strictly-increasing cumulative advances from 0", () => {
        for (const word of DEFAULT_INPUT.words) {
            const advances = (0, layout_1.charAdvances)(word, 32);
    ...
    it("agrees with measureText about the full word width", () => {
        const advances = (0, layout_1.charAdvances)("Inspiration", 32);
        expect(advances[advances.length - 1]).toBeCloseTo((0, template_utils_1.measureText)("Inspiration", { fontSize: 32 }).width, 6);
    });
    it("advances are kern-aware full-word origins (boundary kern included)", () => {
        ...
        const size = 57.76;
        const advances = (0, layout_1.charAdvances)("LT", size);
        expect(advances[1]).toBeLessThan((0, template_utils_1.measureText)("L", { fontSize: size }).width);
        expect(advances[1]).toBeCloseTo((0, template_utils_1.measureText)("LT", { fontSize: size }).width - (0, template_utils_1.measureText)("T", { fontSize: size }).width, 6);
    });
    it("is what the layout's per-word entries carry", () => {
        const layout = (0, layout_1.computeLayout)(DEFAULT_INPUT);
        for (const entry of layout.words) {
            expect(entry.advances).toEqual((0, layout_1.charAdvances)(entry.word, layout.fontSize));
...
describe("computeLayout — real-geometry zones", () => {
    const layout = (0, layout_1.computeLayout)(DEFAULT_INPUT);
    const fillLayout = (0, layout_1.computeLayout)({ ...DEFAULT_INPUT, frame: "fill" });
    it("wordZone pads the word band by the font-proportional ink overhangs", () => {
        const lsb = (0, layout_1.inkLeftPadPx)(layout.fontSize);
        const rsb = (0, layout_1.inkRightPadPx)(layout.fontSize);
        expect(layout.wordZone.x).toBe(Math.max(layout.bar.x, Math.floor(layout.wordX) - lsb - 1));
        expect(layout.wordZone.y).toBe(layout.glyphTop);
        expect(layout.wordZone.h).toBe(layout.glyphBottom - layout.glyphTop);
        expect(layout.wordZone.x + layout.wordZone.w).toBeGreaterThanOrEqual(Math.min(layout.bar.x + layout.bar.w, Math.ceil(layout.wordX + layout.maxWordWidthPx) + rsb + 2));
    });
    it("labelZone exists iff the label shows and includes the underline row", () => {
        ...
        expect(layout.labelZone.y + layout.labelZone.h).toBe(layout.underlineY + layout.underlineHeightPx);
        const hidden = (0, layout_1.computeLayout)({ ...DEFAULT_INPUT, label: "" });
...
describe("computeLayout — determinism", () => {
    it("same input produces an identical layout", () => {
        expect((0, layout_1.computeLayout)(DEFAULT_INPUT)).toEqual((0, layout_1.computeLayout)(DEFAULT_INPUT));
    });
});
