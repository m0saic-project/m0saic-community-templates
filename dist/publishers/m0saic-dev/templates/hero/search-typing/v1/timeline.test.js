...
describe("buildTypingTimeline — natural duration", () => {
    it("sums lead + Σ bands + trail exactly at the defaults (hold)", () => {
        ...
        const t = (0, timeline_1.buildTypingTimeline)(WORDS, TIMING, "hold");
    ...
    it("loop adds the last word's deletes", () => {
        // hold + 11·90 = 19160
        const t = (0, timeline_1.buildTypingTimeline)(WORDS, TIMING, "loop");
        expect(t.durationMs).toBe(19160);
    });
    it("single held word: lead + empty + type + hold + trail", () => {
        const t = (0, timeline_1.buildTypingTimeline)(["Hi"], TIMING, "hold");
            ...
            for (let i = 0; i < band.chars.length; i++) {
                ...
                if (char.deleteAtSec != null) {
                    ...
                    if (i > 0 && band.chars[i - 1].deleteAtSec != null) {
                        expect(char.deleteAtSec).toBeLessThan(band.chars[i - 1].deleteAtSec);
                    }
                }
            }
        }
...
describe("buildTypingTimeline — pinned duration", () => {
    it("rescales every time by the same k and sums to the pinned ms", () => {
        const natural = (0, timeline_1.buildTypingTimeline)(WORDS, TIMING, "loop");
        const pinned = (0, timeline_1.buildTypingTimeline)(WORDS, TIMING, "loop", { pinnedDurationMs: 8000 });
        const k = 8000 / 19160;
        ...
        expect(pinned.bands[2].chars[3].typeAtSec).toBeCloseTo(natural.bands[2].chars[3].typeAtSec * k, 9);
    ...
    it("stretches when pinned past natural (no dead air)", () => {
        const t = (0, timeline_1.buildTypingTimeline)(WORDS, TIMING, "hold", { pinnedDurationMs: 30000 });
    ...
    it("hold keeps the last band pinned to the clip end", () => {
        const t = (0, timeline_1.buildTypingTimeline)(WORDS, TIMING, "hold", { pinnedDurationMs: 8000 });
    ...
    it("fractional naturals still land every time inside [0, durationSec]", () => {
        ...
        const t = (0, timeline_1.buildTypingTimeline)(["Hi"], { ...TIMING, leadMs: 5 }, "loop", { fps: 30 });
        const last = t.bands[t.bands.length - 1];
...
describe("buildTypingTimeline — frame-0 guard", () => {
    ...
    it("clamps a sub-frame lead to one frame", () => {
        const t = (0, timeline_1.buildTypingTimeline)(WORDS, { ...TIMING, leadMs: 5 }, "hold", { fps: 30 });
    ...
    it("uses the given fps (and defaults to 30)", () => {
        const at60 = (0, timeline_1.buildTypingTimeline)(WORDS, { ...TIMING, leadMs: 5 }, "hold", { fps: 60 });
        expect(at60.bands[0].startSec).toBeCloseTo((1000 / 60 / 1000) * at60.scale, 9);
        const noFps = (0, timeline_1.buildTypingTimeline)(WORDS, { ...TIMING, leadMs: 5 }, "hold");
        expect(noFps.bands[0].startSec).toBeCloseTo((1000 / 30 / 1000) * noFps.scale, 9);
    });
    it("leaves an already-long lead untouched", () => {
        const t = (0, timeline_1.buildTypingTimeline)(WORDS, TIMING, "hold", { fps: 30 });
...
describe("buildTypingTimeline — determinism and validation", () => {
    it("same inputs produce an identical timeline", () => {
        const a = (0, timeline_1.buildTypingTimeline)(WORDS, TIMING, "hold", { pinnedDurationMs: 12000, fps: 30 });
        const b = (0, timeline_1.buildTypingTimeline)(WORDS, TIMING, "hold", { pinnedDurationMs: 12000, fps: 30 });
        expect(a).toEqual(b);
    });
    it.each([
        ["empty words", () => (0, timeline_1.buildTypingTimeline)([], TIMING, "hold"), "non-empty"],
        ["empty word", () => (0, timeline_1.buildTypingTimeline)([""], TIMING, "hold"), "non-empty string"],
        [
            "zero timing knob",
            () => (0, timeline_1.buildTypingTimeline)(WORDS, { ...TIMING, typeCharMs: 0 }, "hold"),
            "timing.typeCharMs",
        ],
        [
            "negative timing knob",
            () => (0, timeline_1.buildTypingTimeline)(WORDS, { ...TIMING, trailMs: -1 }, "hold"),
            "timing.trailMs",
        ],
        [
            "bad endBehavior",
            () => (0, timeline_1.buildTypingTimeline)(WORDS, TIMING, "bounce"),
            "endBehavior",
        ],
