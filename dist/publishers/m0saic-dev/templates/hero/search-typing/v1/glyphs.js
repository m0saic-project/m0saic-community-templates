...
exports.RING_SIDES = void 0;
exports.circlePolyPath = circlePolyPath;
exports.magnifierPaths = magnifierPaths;
exports.cardSource = cardSource;
exports.magnifierSource = magnifierSource;
exports.labelChromeSource = labelChromeSource;
exports.wordStripSources = wordStripSources;
const template_utils_1 = require("@m0saic/template-utils");
/** Polygon sides for the magnifier ring circles (arc-free, overlay-safe). */
exports.RING_SIDES = 28;
 ...
 */
function circlePolyPath(cx, cy, radius, sides = exports.RING_SIDES, reverse = false) {
    let d = "";
...
function magnifierPaths(icon) {
    ...
    const outer = circlePolyPath(cx, cy, outerR, exports.RING_SIDES, false);
    const inner = circlePolyPath(cx, cy, innerR, exports.RING_SIDES, true);
    ...
    const handle = `M ${r2(ax + px)} ${r2(ay + py)} ` +
        `L ${r2(ax - px)} ${r2(ay - py)} ` +
        `L ${r2(bx - px)} ${r2(by - py)} ` +
 ...
 */
function cardSource(layout, cardColor) {
    ...
    const rounding = cornerRadiusPx > 0 && cornerRadiusPx >= halfShort - 1
    ...
    return (0, template_utils_1.makeColorTile)(cardColor, { effects: { rounding } });
}
/** The magnifier cell: the 3 polygon subpaths for the EXACT icon square,
 *  authored local to its (snapped) `cell`. */
function magnifierSource(icon, cell, color) {
    return (0, template_utils_1.maskAtlasSource)(magnifierPaths({ x: icon.x - cell.x, y: icon.y - cell.y, w: icon.w, h: icon.h }), color, { width: cell.w, height: cell.h });
}
/**
...
function lineTextPath(text, x, baselineY, layout, cell) {
    return (0, template_utils_1.textToPath)(text, {
 ...
 */
function labelChromeSource(layout, rect, color, opts) {
    const cell = { width: rect.w, height: rect.h };
    const paths = [];
    if (layout.labelText) {
        paths.push(...splitContours(lineTextPath(layout.labelText, layout.labelX - rect.x, layout.baselineY - rect.y, layout, cell)));
    }
    if (opts.underline !== "none") {
        const x0 = layout.labelUnderline.x0;
        const x1 = opts.underline === "static"
            ? layout.wordX + layout.maxWordWidthPx
            : layout.labelUnderline.x1;
        const w = Math.round(x1 - x0);
        if (w >= 1) {
            paths.push((0, template_utils_1.roundedRectPathD)(Math.round(x0) - rect.x, layout.underlineY - rect.y, w, layout.underlineHeightPx, 0));
        }
    }
    const subpaths = paths.filter(Boolean);
    if (subpaths.length === 0)
        return null;
    return (0, template_utils_1.maskAtlasSource)(subpaths, color, cell);
}
/**
 ...
 * `window`.
 */
function wordStripSources(layout, zone, inkColor, windows) {
    const cell = { width: zone.w, height: zone.h };
    return layout.words.map((entry, index) => {
        const source = (0, template_utils_1.makeColorTile)(inkColor, {
            mask: {
                kind: "inline-mask",
                localPath: lineTextPath(entry.word, layout.wordX - zone.x, layout.baselineY - zone.y, layout, cell),
