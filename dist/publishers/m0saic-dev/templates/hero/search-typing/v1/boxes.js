...
exports.CARET_BOX_BUDGET = exports.CARET_GAP_PX = void 0;
exports.charSpans = charSpans;
exports.buildCoverBoxes = buildCoverBoxes;
exports.buildUnderlineBoxes = buildUnderlineBoxes;
exports.buildCaretBoxes = buildCaretBoxes;
const layout_1 = require("./layout");
/** Caret x-gap after the last typed glyph (plan D7). */
exports.CARET_GAP_PX = 3;
...
exports.CARET_BOX_BUDGET = 400;
/** Cover/underline spans for word `wordIndex`, in integer canvas px. */
function charSpans(layout, wordIndex) {
    const { advances } = layout.words[wordIndex];
    const length = advances.length - 1;
    const lsbPad = (0, layout_1.inkLeftPadPx)(layout.fontSize);
    const rsbPad = (0, layout_1.inkRightPadPx)(layout.fontSize);
 ...
 */
function buildCoverBoxes(timeline, layout) {
 ...
 */
function buildUnderlineBoxes(timeline, layout) {
    const boxes = [];
    timeline.bands.forEach((band, k) => {
        const spans = charSpans(layout, k);
        band.chars.forEach((char, i) => {
...
function blinkBoxes(x, fromSec, toSec, blinkSec, rect) {
    const boxes = [];
 ...
 */
function buildCaretBoxes(timeline, layout, caret) {
    const rect = {
        y: layout.glyphTop,
        w: Math.max(1, Math.round(caret.widthPx)),
        h: layout.glyphBottom - layout.glyphTop,
    };
    const posAt = (wordIndex, visibleChars) => Math.round(layout.wordX + layout.words[wordIndex].advances[visibleChars] + exports.CARET_GAP_PX);
    const build = (blinkSec) => {
        const boxes = [];
        timeline.bands.forEach((band, k) => {
            ...
            if (band.deleteStartSec != null) {
                ...
                const deleteAt = (i) => band.chars[i].deleteAtSec;
                boxes.push({
                    x: posAt(k, length),
                    ...rect,
    ...
    for (let guard = 0; boxes.length > exports.CARET_BOX_BUDGET && guard < 24; guard++) {
        period *= 2;
        boxes = build(period);
    }
    ...
    return boxes.map((box) => box.fromSec != null && box.toSec != null && box.toSec - box.fromSec > 0.003
