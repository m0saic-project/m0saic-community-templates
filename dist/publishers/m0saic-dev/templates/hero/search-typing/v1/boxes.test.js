...
const holdTimeline = (0, timeline_1.buildTypingTimeline)(WORDS, TIMING, "hold");
const loopTimeline = (0, timeline_1.buildTypingTimeline)(WORDS, TIMING, "loop");
describe("charSpans", () => {
    it("pads each span by the font-proportional ink overhangs", () => {
        const spans = (0, boxes_1.charSpans)(layout, 0);
        const { advances } = layout.words[0];
        const lsb = (0, layout_2.inkLeftPadPx)(layout.fontSize);
        const rsb = (0, layout_2.inkRightPadPx)(layout.fontSize);
    ...
    it("stays inside the card", () => {
        for (let k = 0; k < WORDS.length; k++) {
            for (const span of (0, boxes_1.charSpans)(layout, k)) {
...
describe("buildCoverBoxes", () => {
    const boxes = (0, boxes_1.buildCoverBoxes)(holdTimeline, layout);
    it("emits 2 boxes per deleting char, 1 per held char, none zero-duration", () => {
        ...
        const loopBoxes = (0, boxes_1.buildCoverBoxes)(loopTimeline, layout);
        expect(loopBoxes).toHaveLength(2 * 41 - 5);
        expect(loopBoxes.length).toBeLessThanOrEqual(template_utils_1.GATED_BOX_BUDGET);
    ...
    it("word 0 typed-covers are pure lt-gates (ON at frame 0)", () => {
        ...
        const band0Typed = boxes.filter((box) => box.fromSec === undefined && box.toSec != null && box.toSec <= band0.typedAtSec + 1e-9);
    ...
    it("later words' typed-covers are between(bandStart, typeAt) — they never cover an earlier word", () => {
        for (let k = 1; k < holdTimeline.bands.length; k++) {
            ...
            const typed = boxes.filter((box) => box.fromSec === band.startSec);
    ...
    it("deleted-covers run from deleteAt to the band end (char 0's skipped)", () => {
        ...
        const deleted = boxes.filter((box) => box.toSec === band.endSec && box.fromSec != null && box.fromSec >= (band.deleteStartSec ?? 0));
        expect(deleted).toHaveLength(band.chars.length - 1);
        deleted.forEach((box) => {
            expect(box.fromSec).toBeGreaterThanOrEqual(band.deleteStartSec);
            expect(box.toSec).toBeGreaterThan(box.fromSec);
        });
    });
...
describe("buildUnderlineBoxes", () => {
    const boxes = (0, boxes_1.buildUnderlineBoxes)(holdTimeline, layout);
...
describe("buildCaretBoxes", () => {
    ...
    const boxes = (0, boxes_1.buildCaretBoxes)(holdTimeline, layout, caret);
    ...
    it("typing carets tile the keystroke windows contiguously", () => {
        ...
        const typing = boxes.filter((box) => box.fromSec != null &&
        ...
        for (let i = 1; i < typing.length; i++) {
            expect(typing[i].fromSec).toBeCloseTo(typing[i - 1].toSec + 0.001, 9);
        }
    });
    it("blinks during holds and stays under its budget", () => {
        expect(boxes.length).toBeLessThanOrEqual(boxes_1.CARET_BOX_BUDGET);
        expect(boxes.length).toBeLessThanOrEqual(template_utils_1.GATED_BOX_BUDGET);
        // At least one blink slot inside the first empty hold.
        const band = holdTimeline.bands[0];
        const blink = boxes.filter((box) => box.fromSec != null && box.fromSec >= band.startSec && (box.toSec ?? 0) <= band.typeStartSec);
        expect(blink.length).toBeGreaterThan(0);
    });
    it("coarsens blink instead of overflowing on very long pinned renders", () => {
        const long = (0, timeline_1.buildTypingTimeline)(WORDS, TIMING, "hold", { pinnedDurationMs: 1_200_000 });
        const longBoxes = (0, boxes_1.buildCaretBoxes)(long, layout, caret);
        expect(longBoxes.length).toBeLessThanOrEqual(template_utils_1.GATED_BOX_BUDGET);
    });
    it("is deterministic", () => {
        expect((0, boxes_1.buildCaretBoxes)(holdTimeline, layout, caret)).toEqual(boxes);
    });
});
describe("track budgets at the schema caps (8 words × 24 chars, loop)", () => {
    ...
    const capLayout = (0, layout_1.computeLayout)({
        canvasW: 1920,
        canvasH: 600,
        words: capWords,
        label: "",
    ...
    it("cover and underline tracks stay under the 500-box wall", () => {
        const t = (0, timeline_1.buildTypingTimeline)(capWords, TIMING, "loop");
        // 192 chars × 2 boxes, minus each word's zero-duration char-0 delete box.
        expect((0, boxes_1.buildCoverBoxes)(t, capLayout)).toHaveLength(2 * 192 - 8);
        expect((0, boxes_1.buildUnderlineBoxes)(t, capLayout)).toHaveLength(192);
    });
    it.each([
        [0, 60],
        [0, 1000],
        ...
        const t = (0, timeline_1.buildTypingTimeline)(capWords, TIMING, "loop", pin > 0 ? { pinnedDurationMs: pin } : {});
        const caretBoxes = (0, boxes_1.buildCaretBoxes)(t, capLayout, { blinkMs, widthPx: 2 });
        expect(caretBoxes.length).toBeLessThanOrEqual(template_utils_1.GATED_BOX_BUDGET);
    });
});
