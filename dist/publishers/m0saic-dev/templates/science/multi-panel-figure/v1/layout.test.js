...
describe("multi-panel-figure layout — m0 emission", () => {
    it("emits one valid cell per panel, label, caption and scale-bar part, in paint order", () => {
        const layout = (0, layout_1.buildFigureLayout)(base());
        expect((0, dsl_1.validateM0String)(String(layout.m0)).ok).toBe(true);
    ...
    it("emits a RATIO m0 — precision bounded by the lattice basis, not the canvas", () => {
        ...
        const metrics = (W, H) => (0, dsl_1.getComplexityMetricsFast)(String((0, layout_1.buildFigureLayout)(base({ W, H })).m0));
        ...
        expect(small.frameCount).toBe((0, layout_1.buildFigureLayout)(base({ W: 1600, H: 1200 })).cells.length);
    });
    it("gutters and margins are exact; panels in a row share a width within 1px", () => {
        const opts = base();
        const layout = (0, layout_1.buildFigureLayout)(opts);
    ...
    it("each label strip sits directly on top of its panel, left-aligned", () => {
        ...
        for (const l of labels) {
            const p = panels.find((c) => c.kind === "panel" && c.panelIndex === l.panelIndex);
    ...
    it("labels off / no footer: only panel cells remain", () => {
        const layout = (0, layout_1.buildFigureLayout)(base({ panelLabels: null, caption: null, scaleBar: null }));
    ...
    it('an empty label ("") skips that panel\'s label cell', () => {
        const layout = (0, layout_1.buildFigureLayout)(base({ rowCounts: [3], panelLabels: ["A", "", "C"], caption: null, scaleBar: null }));
        const labels = layout.cells.filter((c) => c.kind === "label");
    ...
    it("a hopelessly long caption ellipsis-truncates at the minimum font", () => {
        const layout = (0, layout_1.buildFigureLayout)(base({ W: 320, H: 260, rowCounts: [2], panelLabels: null, scaleBar: null, caption: "x".repeat(600) }));
        const cap = layout.cells.find((c) => c.kind === "caption");
    ...
    it("deterministic: identical inputs produce identical layouts", () => {
        expect(JSON.stringify((0, layout_1.buildFigureLayout)(base()))).toBe(JSON.stringify((0, layout_1.buildFigureLayout)(base())));
    });
    it("throws FigureLayoutError on a canvas too small for the rows", () => {
        expect(() => (0, layout_1.buildFigureLayout)(base({ W: 60, H: 40, rowCounts: [4, 4] }))).toThrow(layout_1.FigureLayoutError);
    });
    it("throws FigureLayoutError on a panelLabels length mismatch", () => {
        expect(() => (0, layout_1.buildFigureLayout)(base({ panelLabels: ["A"] }))).toThrow(layout_1.FigureLayoutError);
    });
    it("throws FigureLayoutError when a caption-less scale bar overflows the footer (never a silent squeeze)", () => {
        ...
        expect(() => (0, layout_1.buildFigureLayout)(base({
            W: 400,
            H: 300,
            rowCounts: [1],
            panelLabels: null,
        ...
        }))).toThrow(layout_1.FigureLayoutError);
    });
});
