...
describe("DVD wrap tutorial", () => {
  it("owns a fixed five-page, 25-second production walkthrough", () => {
    const tutorial = renderDvdWrapTutorial(ctx);
    ...
    for (const step of tutorial.steps) expect(isValidM0String((step.file as MosaicDocument).m0)).toBe(true);
  });

  it("teaches variants, artifacts, sidecars, and preflight with ASCII-only copy", () => {
    const tutorial = renderDvdWrapTutorial(ctx);
    const text = tutorial.steps.flatMap((step) => (step.file as MosaicDocument).sources)
      .filter((source): source is MosaicTextSource => source.type === "text")
      .flatMap((source) => source.layers.map((layer) => layer.content.kind === "literal" ? layer.content.text : ""))
      .join(" ");
    for (const concept of ["territory", "WRAP", "PROOF", "sidecar", "CHECK DIGIT", "NO PLACEHOLDERS"]) expect(text).toContain(concept);
