...
import { resolveFacecamReel } from "./facecamReel";
import type { DropCalendarV1Props } from "./types";

const DUR = 30; // seconds

function cfgOf(props: DropCalendarV1Props, durationSec = DUR) {
  const out = resolveDropCalendar(props, durationSec);
...
describe("month/year validation", () => {
  it("hard-fails only on nonsense", () => {
    expect(resolveDropCalendar({ month: 0 }, DUR).ok).toBe(false);
    expect(resolveDropCalendar({ month: 13 }, DUR).ok).toBe(false);
    expect(resolveDropCalendar({ year: 1800 }, DUR).ok).toBe(false);
    expect(resolveDropCalendar({ month: 12, year: 2026 }, DUR).ok).toBe(true);
  });
});
  ...
  it("defensive: numeric-string days tolerated, blank days render nothing silently", () => {
    const { cfg, warnings } = cfgOf({
      ...
      days: [
        ...
        { day: "", title: "parked" },
        { day: "abc", title: "junk" },
      ] as unknown as DropCalendarV1Props["days"],
    });
...
describe("cues against picked facecam clips", () => {
  const props: DropCalendarV1Props = {
    month: 10,
    year: 2026,
    holdSec: 4,
    cues: [
  ...
  it("maps source-stamped cues onto the cut and drops what was cut out", () => {
    const out = resolveDropCalendar(props, DUR, reel);
    ...
    expect(
      out.warnings.some((w) => w.includes("outside the picked facecam clips")),
    ).toBe(true);
  });

  it("carries the reel onto the config and its warnings into the outcome", () => {
    const bad = resolveFacecamReel([{ startMs: 5, endMs: 4 }]);
    const out = resolveDropCalendar({}, DUR, bad);
    if (!out.ok) throw new Error(out.message);
    expect(out.cfg.facecamReel).toBeUndefined();
    expect(out.warnings.some((w) => w.includes("clip #1"))).toBe(true);

    const good = resolveDropCalendar(props, DUR, reel);
  ...
  it("without a reel, cue times stay output times (the pre-clips behavior)", () => {
    const out = resolveDropCalendar(props, DUR);
    if (!out.ok) throw new Error(out.message);
...
describe("classifyMedia", () => {
  it("probe wins, then extension, image as default", () => {
    expect(classifyMedia("x.jpg", "video")).toBe("video");
    expect(classifyMedia("x.mp4", undefined)).toBe("video");
    expect(classifyMedia("x.MOV", undefined)).toBe("video");
    expect(classifyMedia("x.png", undefined)).toBe("image");
    expect(classifyMedia("https://a/b.webm?tok=1", undefined)).toBe("video");
    expect(classifyMedia("mystery", undefined)).toBe("image");
  });
});
