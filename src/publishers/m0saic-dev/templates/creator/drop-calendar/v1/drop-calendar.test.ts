...
import { assertLayout, engineRecover, resolvePropBindings } from "@m0saic/template-utils";

import { DropCalendarV1 } from "./drop-calendar";
...
import { platformStage, resolveCreatorPlatform } from "./platforms";
...
function paintedRect(doc: any, parsedFrames: any[], source: any) {
  const f = parsedFrames[doc.sources.indexOf(source)];
  return engineRecover(
    { x: f.x, y: f.y, w: f.width, h: f.height },
    source.placement?.inset ?? null,
  );
}
...
async function render(props: DropCalendarV1Props, ctx = makeCtx()): Promise<any> {
  return (await DropCalendarV1.render(props, ctx)) as any;
}
...
describe("zero-props render (the demo contract)", () => {
  it("renders the October 2026 demo calendar as a static PNG doc", async () => {
    const doc = await render({ ...DropCalendarV1.defaultProps });
    ...
    const parsed = parseM0StringComplete(doc.m0, 1920, 1080);
    ...
    expect(labels.filter((l: string) => l === "header-cell")).toHaveLength(7);
    expect(labels.filter((l: string) => l === "drop-cell")).toHaveLength(6);
    ...
    const masthead = doc.sources.find((s: any) => s.editor?.label === "masthead");
...
describe("layout contract sweep (masthead fits, sheet + slab present)", () => {
  ...
  for (const [w, h] of CANVASES) {
    it(`${w}x${h}`, async () => {
      ...
      const doc = await render({ ...DropCalendarV1.defaultProps, platform: "canvas" }, ctx);
      expect(errorCode(doc)).toBeUndefined();
      assertLayout(doc, ctx, "@m0saic-dev/creator/drop-calendar/v1", {
        constraints: [
...
describe("drops with media", () => {
  it("teaser images fill cells; number badge + title strip ride on top", async () => {
    ...
    expect(errorCode(doc)).toBeUndefined();
    const media = doc.sources.filter((s: any) => s.type === "media");
  ...
  it("a teaser VIDEO makes the doc a video and loops muted", async () => {
    ...
    const media = doc.sources.find((s: any) => s.type === "media");
...
describe("facecam", () => {
  it("side-by-side on desktop, facecam audio on, duration from the probe", async () => {
    ...
    const doc = await render(
      { ...DropCalendarV1.defaultProps, facecam: "cam.mp4" },
      ctx,
    );
    expect(errorCode(doc)).toBeUndefined();
    expect(doc.durationMs).toBe(42000);
    expect(doc.format.kind).toBe("video");
    const cam = doc.sources.find((s: any) => s.editor?.label === "facecam");
    ...
    if (parsed.ok) {
      ...
      const paper = paintedRect(doc, parsed.ir.renderFrames, doc.sources.find((s: any) => s.editor?.label === "paper"));
  ...
  it("portrait: the calendar is the video and the facecam is a top-left picture-in-picture", async () => {
    const ctx = makeCtx(
      ...
      { "cam.mp4": { kind: "video", durationMs: 20000 } },
    );
    ...
    if (parsed.ok) {
      ...
      const paper = paintedRect(doc, parsed.ir.renderFrames, doc.sources.find((s: any) => s.editor?.label === "panel"));
...
describe("facecam clips (the picked regions)", () => {
  ...
  it("one clip trims the facecam in place and sets the render length", async () => {
    const doc = await render(
      { facecam: "cam.mp4", facecamClips: [{ startMs: 12000, endMs: 20000 }] },
      camCtx(),
    );
    expect(errorCode(doc)).toBeUndefined();
  ...
  it("several clips become a stitched reel child with the authored transition", async () => {
    ...
    expect(reel.steps.map((s: any) => s.durationMs)).toEqual([5000, 3000, 2000]);
    expect(reel.steps.map((s: any) => s.transitionToNext)).toEqual([
  ...
  it('"cut" joins the clips with no transition and no overlap', async () => {
    ...
    expect(doc.durationMs).toBe(8000);
    expect(
      doc.children.dc_facecam_reel.steps.map((s: any) => s.transitionToNext),
  ...
  it("an EXPLICIT duration ask wins over the picked clips; a hint-seeded target does not", async () => {
    ...
    const asked = await render(
      { facecam: "cam.mp4", facecamClips: [{ startMs: 0, endMs: 3000 }] },
      { ...makeCtx({ durationMs: 20000 }, media), userIntent: { durationMs: 20000 } } as unknown as MosaicEngineContext,
    );
    expect(asked.durationMs).toBe(20000);
    const seeded = await render(
      { facecam: "cam.mp4", facecamClips: [{ startMs: 0, endMs: 3000 }] },
      makeCtx({ durationMs: 20000 }, media),
    );
  ...
  it("carries talk-track cues from facecam time onto the cut timeline", async () => {
    ...
    expect(errorCode(doc)).toBeUndefined();
    const rings = doc.sources.filter((s: any) => s.editor?.label === "cue-ring");
    // Two surviving cues × 4 ring bars — the cut-out cue is gone.
    expect(rings).toHaveLength(8);
    const windows = [...new Set(rings.map((r: any) => r.overlay.enable))];
...
describe("cues", () => {
  const cueProps: DropCalendarV1Props = {
    month: 10,
    year: 2026,
    days: [{ day: 13, title: "New set" }],
    teasers: ["t.jpg"],
  ...
  it("ring-highlights the cued day and spotlights its teaser", async () => {
    ...
    const spot = doc.sources.find((s: any) => s.editor?.label === "spotlight");
    ...
    const frame = doc.sources.find((s: any) => s.editor?.label === "spotlight-frame");
    expect(frame.overlay.enable).toBe("between(t,3.000,6.000)");
    const dim = doc.sources.find((s: any) => s.editor?.label === "spotlight-dim");
  ...
  it("zoom delay: 0 zooms with the ring; a cue shorter than the delay still gets a short zoom", async () => {
    ...
    expect(now.sources.find((s: any) => s.editor?.label === "spotlight").overlay.enable).toBe("between(t,2.000,6.000)");
    const short = await render(
      { ...cueProps, cues: [{ text: "13", startMs: 2000, endMs: 2600 }], spotlight: { delaySec: 3 } },
      ctx,
    );
    const spot = short.sources.find((s: any) => s.editor?.label === "spotlight");
    expect(spot.overlay.enable).toBe("between(t,2.200,2.600)");
    for (const r of short.sources.filter((s: any) => s.editor?.label === "cue-ring")) {
  ...
  it("spotlight off → rings only", async () => {
    const doc = await render(
      { ...cueProps, spotlight: { enabled: false } },
      makeCtx({ durationMs: 30000 }),
    );
    expect(doc.sources.filter((s: any) => s.editor?.label === "cue-ring")).toHaveLength(4);
    expect(doc.sources.find((s: any) => s.editor?.label === "spotlight")).toBeUndefined();
    expect(doc.sources.find((s: any) => s.editor?.label === "spotlight-dim")).toBeUndefined();
  });
...
describe("canvas prop bindings (double-click to edit)", () => {
  it("a text drop cell opens as a Day · Title form routed to its ORIGINAL row", async () => {
    ...
    const cell = doc.sources.find(
      (s: any) =>
        s.editor?.label === "drop-cell" &&
        s.layers?.some((l: any) => l.content.text === "13"),
    );
  ...
  function headersByColumn(doc: any, w: number, h: number) {
    const parsed = parseM0StringComplete(doc.m0, w, h);
    if (!parsed.ok) throw new Error("parse failed");
    return doc.sources
      .filter((s: any) => s.editor?.label === "header-cell")
      .sort(
        (a: any, b: any) =>
          paintedRect(doc, parsed.ir.renderFrames, a).x -
  ...
  it("empty dates are ADD handles: Day · Title routed to the next free row", async () => {
    ...
    // cells stay unbound.
    const dayCells = doc.sources.filter((s: any) => s.editor?.label === "day-cell");
    const bound = dayCells.filter((s: any) => s.editor?.bindings);
    ...
    for (const cell of bound) {
      const dayText = cell.layers[0].content.text as string;
    ...
    expect(seeds.sort((a, b) => a - b)).toEqual(
      Array.from({ length: 31 }, (_, i) => i + 1).filter((d) => !dropDays.has(d)),
    );
  });
  ...
  it("facecamSlot: with no facecam the slot shows the included starter in the side-by-side layout, bound to move AND to take a drop", async () => {
    const off = await render({ ...DropCalendarV1.defaultProps });
    expect(off.sources.find((s: any) => s.editor?.label === "facecam")).toBeUndefined();

    const on = await render({ ...DropCalendarV1.defaultProps, facecamSlot: true });
    const cam = on.sources.find((s: any) => s.editor?.label === "facecam");
    ...
    expect(on.sources.find((s: any) => s.editor?.label === "facecam-frame")).toBeDefined();

    // A real facecam outranks the slot: the starter never shows beside it.
    const real = await render({ ...DropCalendarV1.defaultProps, facecamSlot: true, facecam: "cam.mp4" });
    const realCam = real.sources.find((s: any) => s.editor?.label === "facecam");
  ...
  it("every weekday header — branded or plain — edits its weekdays.<day> field", async () => {
    ...
    expect(headers.map((s: any) => s.editor.binding.propKey)).toEqual([
      "weekdays.sunday",
      "weekdays.monday",
      "weekdays.tuesday",
      "weekdays.wednesday",
    ...
    // the weekday name.
    const textOf = (s: any) => s.layers.map((l: any) => l.content.text).join(" ");
  ...
  it("monday week start keeps each header bound to its own weekday", async () => {
    const doc = await render({ ...DropCalendarV1.defaultProps, weekStart: "monday" });
    const headers = headersByColumn(doc, 1920, 1080);
    ...
    expect(
      headers[5].layers.map((l: any) => l.content.text).join(" "),
    ).toBe("STREAM SATURDAY");
  });
  ...
  it("a media cell's badge edits the day, its strip edits the title, and the cell itself is the teaser slot's drop target", async () => {
    ...
    const cell = doc.sources.find((s: any) => s.editor?.label === "drop-cell" && s.type === "media");
    expect(cell.editor.binding).toEqual({ propKey: "teasers", index: 0 });
    const { rejected, byProp } = resolvePropBindings(doc, 1920, 1080, { propsSchema: DropCalendarV1.propsSchema as any });
    ...
    const badge = doc.sources.find((s: any) => s.editor?.label === "day-badge");
    expect(badge.editor.bindings).toEqual([
      { propKey: "days", path: [0, "day"], kind: "number", onClear: "remove-element" },
    ]);
    const strip = doc.sources.find((s: any) => s.editor?.label === "title-strip");
...
describe("geometry: inset recovery, always", () => {
  it("the m0 GCD-collapses (~6x vs exact rects) and pieces recover via insets", async () => {
    ...
    expect(isValidM0String(doc.m0)).toBe(true);
    expect(doc.sources.some((s: any) => s.placement?.inset)).toBe(true);
  });

  it("recovered pieces paint on their exact computed rects", async () => {
    const exact = await render({ ...DropCalendarV1.defaultProps });
    const compact = exact;
    // Same painted geometry either way — the render doesn't care.
    const painted = (doc: any) => {
      const parsed = parseM0StringComplete(doc.m0, 1920, 1080);
      if (!parsed.ok) throw new Error("parse failed");
      return doc.sources
        .map((s: any) => ({
        ...
        .sort(
          (a: any, b: any) =>
            a.label.localeCompare(b.label) || a.x - b.x || a.y - b.y,
        );
    };
  ...
  it("every piece paints its EXACT rect at awkward canvases too", () => {
    ...
    const resolved = resolveDropCalendar(
      {
        ...DropCalendarV1.defaultProps,
        facecam: "cam.mp4",
        teasers: ["a.jpg", "b.mp4"],
    ...
    if (!resolved.ok) return;
    {
      for (const [w, h] of canvases) {
        const built = buildDropCalendarDoc(
          ...
          30,
          30000,
          (p) => classifyMedia(p, undefined),
        );
        expect(built.ok).toBe(true);
        if (!built.ok) continue;
        const parsed = parseM0StringComplete(built.doc.m0, w, h);
        expect(parsed.ok).toBe(true);
...
describe("platform — first-class output selection", () => {
  ...
  async function renderAt(props: DropCalendarV1Props, media?: Record<string, { kind?: string; durationMs?: number }>) {
    const hints = (DropCalendarV1.resolveOutputHints as (p: DropCalendarV1Props) => { width?: number; height?: number })(props);
    const target = hints.width && hints.height ? { width: hints.width, height: hints.height } : {};
    return render(props, makeCtx(target, media));
  }
  function painted(doc: any, label: string) {
    const parsed = parseM0StringComplete(doc.m0, doc.size.width, doc.size.height);
    if (!parsed.ok) throw new Error("m0 did not parse");
    const src = doc.sources.find((s: any) => s.editor?.label === label);
  ...
  it("resolveOutputHints: the platform picks the canvas hosts seed the target from", () => {
    const resolve = DropCalendarV1.resolveOutputHints as (p: DropCalendarV1Props) => Record<string, unknown>;
    ...
    expect(resolve(DropCalendarV1.defaultProps as DropCalendarV1Props)).toEqual({ width: 1920, height: 1080 });
  });
  ...
  it("renders at the host's target — the platform shapes the safe area, not the size", async () => {
    ...
    expect(isValidM0String(tall.m0)).toBe(true);
  ...
  for (const platform of ["tiktok", "youtube-shorts", "instagram-reel"]) {
    it(`${platform}: the card stays inside the chrome and clear of the action rail`, async () => {
      ...
      expect(errorCode(doc)).toBeUndefined();
      const { safe, rail } = platformStage(resolveCreatorPlatform(platform), 1080, 1920);
      ...
      expect(panel.x + panel.w).toBeLessThanOrEqual((rail as { x: number }).x);
    });
  }

  it("a story has chrome but no rail: the slab spans the safe width", async () => {
    const doc = await renderAt({ platform: "instagram-story" });
    const { safe } = platformStage(resolveCreatorPlatform("instagram-story"), 1080, 1920);
    const panel = painted(doc, "panel");
  ...
  it("with a facecam on a rail platform the calendar is the video and the PiP keeps its corner", async () => {
    ...
    const { safe, rail } = platformStage(resolveCreatorPlatform("tiktok"), 1080, 1920);
    ...
    expect(pip.x + pip.w).toBeLessThanOrEqual((rail as { x: number }).x);
    ...
    expect(pipR.y + pipR.h).toBeLessThanOrEqual((rail as { y: number }).y);
  });
  ...
  it("portrait and square reflow titles into a drops list under the grid; desktop keeps them in the cells", async () => {
    for (const platform of ["tiktok", "instagram-story", "instagram-post", "instagram-portrait"]) {
      const doc = await renderAt({ ...DropCalendarV1.defaultProps, platform });
      const rows = doc.sources.filter((s: any) => s.editor?.label === "drop-row");
      ...
      // row a cell would.
      const first = rows.find((r: any) => r.layers[0].content.text === "02");
      expect(first.editor.bindings).toEqual([
        { propKey: "days", path: [0, "day"], kind: "number", onClear: "remove-element" },
        { propKey: "days", path: [0, "title"], kind: "string" },
      ]);
      // Rows read "02  NEW SINGLE OUT", and no cell carries a title.
      expect(first.layers.map((l: any) => l.content.text)).toEqual(["02", "NEW SINGLE OUT"]);
      const cells = doc.sources.filter((s: any) => s.editor?.label === "drop-cell");
      for (const c of cells) expect(c.layers.length).toBe(1);
    ...
    for (const platform of ["youtube", "twitch", "x"]) {
      const doc = await renderAt({ ...DropCalendarV1.defaultProps, platform });
      expect(doc.sources.some((s: any) => s.editor?.label === "drop-row")).toBe(false);
      const titled = doc.sources.filter((s: any) => s.editor?.label === "drop-cell" && s.layers.length > 1);
  ...
  it("cell titles never paint past the cell, even at a small desktop canvas", async () => {
    const doc = await render(
      { platform: "canvas", days: [{ day: 13, title: "Extraordinarily long premiere" }] },
      makeCtx({ width: 854, height: 480 }),
    );
    expect(errorCode(doc)).toBeUndefined();
    const parsed = parseM0StringComplete(doc.m0, 854, 480);
    if (!parsed.ok) throw new Error("m0 did not parse");
    const cell = doc.sources.find((s: any) => s.editor?.label === "drop-cell");
...
describe("safe area switch + mock platform chrome", () => {
  ...
  it("safeArea: false uses the whole canvas (slab inside a plain margin, no rail avoidance)", async () => {
    const on = await render({ ...DropCalendarV1.defaultProps, platform: "tiktok" }, tall());
    const off = await render({ ...DropCalendarV1.defaultProps, platform: "tiktok", safeArea: false }, tall());
  ...
  it("mock chrome is off by default; on TikTok it paints the top bar, caption area, rail and 4 buttons last", async () => {
    const off = await render({ ...DropCalendarV1.defaultProps, platform: "tiktok" }, tall());
    expect(off.sources.some((s: any) => s.editor?.label === "chrome")).toBe(false);
    const on = await render({ ...DropCalendarV1.defaultProps, platform: "tiktok", showChrome: true }, tall());
    expect(errorCode(on)).toBeUndefined();
    const chrome = on.sources.filter((s: any) => s.editor?.label === "chrome");
    expect(chrome.length).toBe(7);
    expect(isValidM0String(on.m0)).toBe(true);
    expect(on.sources.indexOf(chrome[0])).toBeGreaterThan(on.sources.findIndex((s: any) => s.editor?.label === "masthead"));
  });

  it("mock chrome does nothing without chrome (YouTube) or with the safe area off", async () => {
    const yt = await render({ ...DropCalendarV1.defaultProps, platform: "youtube", showChrome: true });
    expect(yt.sources.some((s: any) => s.editor?.label === "chrome")).toBe(false);
    const off = await render({ ...DropCalendarV1.defaultProps, platform: "tiktok", showChrome: true, safeArea: false }, tall());
    expect(off.sources.some((s: any) => s.editor?.label === "chrome")).toBe(false);
  });
});
...
describe("facecam area (drawn rect escape hatch)", () => {
  ...
  it("a drawn rect moves ONLY the facecam — the calendar keeps its side-by-side layout (landscape)", async () => {
    const auto = await render({ ...DropCalendarV1.defaultProps, facecam: "cam.mp4" }, makeCtx({}, media));
    const doc = await render(
      { ...DropCalendarV1.defaultProps, facecam: "cam.mp4", facecamRegion: { regions: [{ x: 1400, y: 120, w: 420, h: 300 }] } },
      makeCtx({}, media),
    );
    expect(errorCode(doc)).toBeUndefined();
  ...
  it("rescales from the canvas it was drawn on, on any platform", async () => {
    ...
    expect(doc.sources.filter((s: any) => s.editor?.label === "facecam")).toHaveLength(1);
  });

  it("junk or an off-canvas rect falls back to automatic placement", async () => {
    const auto = await render({ ...DropCalendarV1.defaultProps, facecam: "cam.mp4" }, makeCtx({}, media));
    const junk = await render({ ...DropCalendarV1.defaultProps, facecam: "cam.mp4", facecamRegion: "{not json" }, makeCtx({}, media));
    expect(painted(junk, "facecam")).toEqual(painted(auto, "facecam"));
    const off = await render(
      { ...DropCalendarV1.defaultProps, facecam: "cam.mp4", facecamRegion: { regions: [{ x: 5000, y: 5000, w: 10, h: 10 }] } },
      makeCtx({}, media),
    );
  ...
  it("a drawn rect without a facecam is inert", async () => {
    const doc = await render({ ...DropCalendarV1.defaultProps, facecamRegion: { regions: [{ x: 10, y: 10, w: 100, h: 100 }] } });
    expect(doc.sources.some((s: any) => s.editor?.label === "facecam")).toBe(false);
  });
});
...
describe("facecam frame", () => {
  ...
  it("a thin theme-coloured frame sits just outside the facecam cell (default 2 px; thickness + colour are knobs)", async () => {
    const doc = await render({ ...DropCalendarV1.defaultProps, facecam: "cam.mp4" }, makeCtx({}, media));
    const cam = painted(doc, "facecam");
    const frame = painted(doc, "facecam-frame");
    expect(frame).toEqual({ x: cam.x - 2, y: cam.y - 2, w: cam.w + 4, h: cam.h + 4 });
    const src = doc.sources.find((s: any) => s.editor?.label === "facecam-frame");
    expect(src.visual?.backgroundColor ?? src.color).toBe("#C4B5FD"); // studio frame
    // Painted under the facecam, above the sheet.
    expect(doc.sources.indexOf(src)).toBeLessThan(doc.sources.findIndex((s: any) => s.editor?.label === "facecam"));

    const thick = await render({ ...DropCalendarV1.defaultProps, facecam: "cam.mp4", facecamStrokePx: 6, facecamStrokeColor: "#FF8800" }, makeCtx({}, media));
    const camT = painted(thick, "facecam");
    const frameT = painted(thick, "facecam-frame");
    expect(frameT.w - camT.w).toBe(12);
    const srcT = thick.sources.find((s: any) => s.editor?.label === "facecam-frame");
    expect(srcT.visual?.backgroundColor ?? srcT.color).toBe("#FF8800");
    const none = await render({ ...DropCalendarV1.defaultProps, facecam: "cam.mp4", facecamStrokePx: 0 }, makeCtx({}, media));
    expect(none.sources.some((s: any) => s.editor?.label === "facecam-frame")).toBe(false);
  });
  it("no facecam → no frame", async () => {
    const doc = await render({ ...DropCalendarV1.defaultProps });
    expect(doc.sources.some((s: any) => s.editor?.label === "facecam-frame")).toBe(false);
  });
});

describe("spotlight is the hero: centred, with the facecam floating above it", () => {
  const media = { "cam.mp4": { kind: "video", durationMs: 20000 } };
  const base: DropCalendarV1Props = {
    ...DropCalendarV1.defaultProps,
    platform: "tiktok",
    safeArea: false,
    month: 10,
  ...
  it("a facecam drawn over the zoom centre does not move the zoom", async () => {
    ...
    const doc = await render(
      { ...base, facecam: "cam.mp4", facecamRegion: { canvas: { w: 1080, h: 1920 }, regions: [{ x: 72, y: 333, w: 265, h: 460 }] } },
      ctx,
    );
    expect(errorCode(doc)).toBeUndefined();
  ...
  it("the facecam and its frame paint above the spotlight, its frame and the dim wash", async () => {
    ...
    const order = doc.sources.map((s: any) => s.editor?.label);
