...
const inside = (r: Rect, W: number, H: number) =>
  r.x >= 0 && r.y >= 0 && r.x + r.w <= W && r.y + r.h <= H && r.w >= 1 && r.h >= 1;
...
describe("bold face", () => {
  it("resolves the bundled Roboto Bold and measures wider than regular-ish sizes", () => {
    expect(boldFontPath()).toMatch(/Roboto-Bold\.ttf$/);
    expect(textWidth("VIDEO", 100)).toBeGreaterThan(250);
...
describe("stickerLine", () => {
  it("hugs the cap box plus the outline pad, baseline at cap-top + cap height", () => {
    const l = stickerLine("VIDEO", 160, 540, 300, true);
    expect(l.haloPx).toBe(Math.round(160 * HALO_EM));
    expect(l.strokePx).toBe(Math.round(160 * STROKE_EM));
    const pad = l.haloPx + 2;
    expect(l.cell.y).toBe(300 - pad);
    expect(l.cell.h).toBe(Math.round(160 * CAP_HEIGHT_EM) + 2 * pad);
    expect(l.baselineY).toBe(Math.round(300 + 160 * CAP_HEIGHT_EM));
  ...
  it("plain lines carry no outline; descender glyphs get a deeper cell", () => {
    const plain = stickerLine("YOUTU.BE", 50, 540, 700, false);
    expect(plain.haloPx).toBe(0);
    expect(plain.strokePx).toBe(0);
    const q = stickerLine("QUICK", 100, 540, 0, false);
    const n = stickerLine("NEW", 100, 540, 0, false);
...
describe("computeStoryLayout at the story canvas (1080×1920)", () => {
  const W = 1080;
  const H = 1920;
  const L = computeStoryLayout(W, H, REF);

  it("scale 1, design column = the canvas", () => {
    expect(L.scale).toBe(1);
    expect(L.stage).toEqual({ x: 0, y: 0, w: DESIGN_W, h: DESIGN_H });
  });
  ...
  it("reads top to bottom: headline → CTA → arrows → pill → screenshot", () => {
    ...
    for (const a of L.arrows) expect(a.y).toBeGreaterThanOrEqual(bottom(L.ctaBox));
...
describe("computeStoryLayout — variants", () => {
  it("a drawn screenshot rect replaces the default slot verbatim", () => {
    const L = computeStoryLayout(1080, 1920, { ...REF, mediaOverride: { x: 100, y: 1200, w: 880, h: 500 } });
  ...
  it("no badge: the small line centres on the canvas", () => {
    const L = computeStoryLayout(1080, 1920, { ...REF, hasBadge: false });
  ...
  it("single-word headline: no small line, the badge sits beside the big line", () => {
    const L = computeStoryLayout(1080, 1920, { ...REF, headlineTop: "", headlineMain: "PREMIERE" });
  ...
  it("long copy shrinks to fit the column instead of spilling", () => {
    const L = computeStoryLayout(1080, 1920, {
  ...
  it("scales with the shorter design ratio and centres the column on a wide canvas", () => {
    const half = computeStoryLayout(540, 960, REF);
    ...
    const wide = computeStoryLayout(1920, 1080, REF);
  ...
  it("is deterministic", () => {
    expect(computeStoryLayout(1080, 1920, REF)).toEqual(computeStoryLayout(1080, 1920, REF));
  });
});
