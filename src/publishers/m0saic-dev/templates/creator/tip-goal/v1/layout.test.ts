...
import {
  computeTipGoalGeometry,
  validateGeometryOverride,
  validateLayoutM0,
  type TipGoalLook,
} from "./layout";

const LOOK: TipGoalLook = {
  placement: "bottom",
  widthFrac: 0.94,
  heightFrac: 0.11,
  marginFrac: 0.05,
...
describe("preset geometry (1920x1080)", () => {
  it("splits a bottom band into label | bar without overlap", () => {
    const out = computeTipGoalGeometry(1920, 1080, LOOK, "$100", undefined, undefined);
    if (!out.ok) throw new Error(out.message);
    const { barRect, labelRect, fontPx } = out.geom;
    if (!labelRect) throw new Error("expected a label rect");
  ...
  it('label "none" → the bar takes the whole band', () => {
    const out = computeTipGoalGeometry(
      1920,
      1080,
      { ...LOOK, labelPlacement: "none" },
      "$100",
      undefined,
      undefined,
    );
  ...
  it('label "above" stacks counter over bar', () => {
    const out = computeTipGoalGeometry(
      1920,
      1080,
      { ...LOOK, labelPlacement: "above", heightFrac: 0.2 },
      "$100",
      undefined,
      undefined,
    );
  ...
  it("a longer sample string fits a smaller font", () => {
    const short = computeTipGoalGeometry(1920, 1080, LOOK, "$9", undefined, undefined);
    const long = computeTipGoalGeometry(1920, 1080, LOOK, "$100000 / $200000", undefined, undefined);
  ...
  it("a tiny canvas → TG_CANVAS_TOO_SMALL", () => {
    const out = computeTipGoalGeometry(60, 40, LOOK, "$100", undefined, undefined);
    expect(out.ok).toBe(false);
    if (!out.ok) expect(out.code).toBe("TG_CANVAS_TOO_SMALL");
  });
});

describe("overrides", () => {
  it("geometry rects and fontSizePx win over the preset", () => {
    const out = computeTipGoalGeometry(1920, 1080, LOOK, "$100", undefined, {
  ...
  it("layoutM0 whole-band replacement drives the split", () => {
    const m0 = validateLayoutM0("5[F,-,-,-,-]", 1920, 1080);
    if (m0.kind !== "rect") throw new Error("expected a rect");
    const out = computeTipGoalGeometry(1920, 1080, LOOK, "$100", m0.rect, undefined);
    if (!out.ok) throw new Error(out.message);
...
describe("layoutM0 validation", () => {
  it("empty / comments-only → unset", () => {
    expect(validateLayoutM0(undefined, 1920, 1080).kind).toBe("unset");
    expect(validateLayoutM0("  # just a comment\n", 1920, 1080).kind).toBe("unset");
  });

  it("garbage → TG_M0_PARSE", () => {
    const out = validateLayoutM0("(((", 1920, 1080);
    expect(out.kind).toBe("error");
    if (out.kind === "error") expect(out.code).toBe("TG_M0_PARSE");
  });

  it("two rects → TG_M0_COUNT", () => {
    const out = validateLayoutM0("2(1,1)", 1920, 1080);
    expect(out.kind).toBe("error");
    if (out.kind === "error") expect(out.code).toBe("TG_M0_COUNT");
  });

  it("a tall rect → TG_M0_SHAPE", () => {
    const out = validateLayoutM0("1", 1920, 1080);
    expect(out.kind).toBe("error");
    if (out.kind === "error") expect(out.code).toBe("TG_M0_SHAPE");
  });
});

describe("geometry-override validation", () => {
  it("nullish → empty", () => {
    const out = validateGeometryOverride(undefined);
    if (!out.ok) throw new Error(out.message);
    expect(out.value).toEqual({});
  });

  it("an array → TG_GEOMETRY_PARSE", () => {
    const out = validateGeometryOverride([1]);
    expect(out.ok).toBe(false);
    if (!out.ok) expect(out.code).toBe("TG_GEOMETRY_PARSE");
  });

  it("a bad rect → TG_GEOMETRY", () => {
    const out = validateGeometryOverride({ barRect: { x: 0, y: 0, w: -1, h: 5 } });
    expect(out.ok).toBe(false);
    if (!out.ok) expect(out.code).toBe("TG_GEOMETRY");
  });

  it("a junk fontSizePx degrades with a warning", () => {
    const out = validateGeometryOverride({ fontSizePx: "big" });
    if (!out.ok) throw new Error(out.message);
