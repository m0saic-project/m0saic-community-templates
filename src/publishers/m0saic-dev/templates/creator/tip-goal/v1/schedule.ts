...
export const MIN_STEP_SEC = 0.02;
/** Tip-flash gate length, seconds. */
export const FLASH_SEC = 0.25;

const RISE_EASES: RiseEase[] = ["linear", "easeOut", "smoothstep", "easeInOut"];
const CURVES: TipGoalCurve[] = ["steady", "big-finish", "fast-start"];

export type AmountAnchor = { t: number; v: number; ease?: EaseName };
...
export function sanitizeRiseEase(v: unknown, def: RiseEase = "easeOut"): RiseEase {
  return typeof v === "string" && (RISE_EASES as string[]).includes(v)
    ? (v as RiseEase)
    : def;
}
...
export function resolveSchedule(
  ...
  if (input.schedule !== undefined && input.schedule !== null) {
    if (!Array.isArray(input.schedule)) {
      return err(
        "TG_SCHEDULE_PARSE",
        "Schedule must be an array like [{ atSec, amount, ease? }].",
      );
    }
  ...
  if (input.tips !== undefined && input.tips !== null) {
    ...
    if (input.tips.length > 0) {
      ...
      if (tips.length === 0) {
        return err(
          "TG_TIPS_PARSE",
          "Every Tips row was invalid — each needs a finite atSec and a nonzero finite amount.",
        );
      }
...
function sanitizeTips(rows: unknown[], warnings: string[]): TipGoalTip[] {
  const tips: TipGoalTip[] = [];
  let dropped = 0;
  for (const row of rows) {
    const r = row as { atSec?: unknown; amount?: unknown };
 ...
 * the board negative.
 */
export function anchorsFromTips(
  tips: TipGoalTip[],
  startAmount: number,
  riseSec: number,
  riseEase: RiseEase,
  ...
  if (tips.length > MAX_TIPS) {
    return err(
      "TG_TOO_DENSE",
      `${tips.length} tips exceed the ${MAX_TIPS}-tip budget — thin the list or use the Schedule prop with fewer keys.`,
    );
  }
  ...
  const anchors: AmountAnchor[] = [{ t: 0, v: Math.max(0, startAmount) }];
  const tipTimes: number[] = [];
  let running = Math.max(0, startAmount);
  let lastEnd = 0;

  for (let i = 0; i < sorted.length; i++) {
    const tStart = round3(Math.max(sorted[i].atSec, lastEnd + MIN_STEP_SEC));
    const nextAt = i + 1 < sorted.length ? sorted[i + 1].atSec : Infinity;
    const maxRise = Math.max(0.001, nextAt - tStart - MIN_STEP_SEC);
...
function anchorsFromSchedule(rows: unknown[], warnings: string[]): ScheduleOutcome {
  if (rows.length > MAX_ANCHORS) {
    return err(
      "TG_TOO_DENSE",
      `${rows.length} schedule keys exceed the ${MAX_ANCHORS}-key budget.`,
    );
  }
  const keys: AmountAnchor[] = [];
  let dropped = 0;
  for (const row of rows) {
    const r = row as { atSec?: unknown; amount?: unknown; ease?: unknown };
    const atSec = typeof r?.atSec === "number" && Number.isFinite(r.atSec) ? r.atSec : null;
    ...
    const ease =
      r?.ease === undefined ? "linear" : sanitizeRiseEase(r.ease, "linear");
  ...
  if (keys.length === 0) {
    return err(
      "TG_SCHEDULE_PARSE",
      "Every schedule key was invalid — each needs finite atSec and amount.",
    );
  }
  ...
  for (let i = 1; i < keys.length; i++) {
    if (keys[i].v > keys[i - 1].v) tipTimes.push(keys[i - 1].t);
  }
  return {
    ok: true,
    anchors: keys,
 ...
 */
export function generateAutoTips(
  auto: TipGoalAuto,
  startAmount: number,
  goalAmount: number,
  durationSec: number,
  ...
  const tipCount = Math.max(1, Math.min(MAX_TIPS, requested));
  if (requested !== tipCount && Number.isFinite(requested)) {
    warnings.push(`Auto tip count ${requested} clamped to ${tipCount}.`);
  }
  const curve: TipGoalCurve =
    typeof auto.curve === "string" && (CURVES as string[]).includes(auto.curve)
      ? auto.curve
      : "big-finish";
  ...
  if (!(total > 0)) {
    warnings.push(
      `Start amount (${startAmount}) already meets the goal (${goalAmount}) — the bar holds flat.`,
    );
    return [];
  }

  let n = tipCount;
  if (total < n) n = Math.max(1, Math.floor(total));
  ...
  const weights = times.map((t) => {
    ...
    if (curve === "big-finish") w *= 0.55 + u;
    else if (curve === "fast-start") w *= 1.55 - u;
    return w;
  });
 ...
 */
export function buildFillXExpr(
  anchors: AmountAnchor[],
  goalAmount: number,
): string {
  ...
  const raw = keyframeExpr(keys, { ease: "linear", precision: 5 });
  return `-(w*(1-(${rebalanceAdditiveChains(raw)})))`;
}
 ...
 */
export function buildCounterTextExpr(
  anchors: AmountAnchor[],
  prefix: string,
  suffix: string,
): string {
  const raw = keyframeExpr(
    anchors.map((a) => ({ t: a.t, v: a.v, ...(a.ease !== undefined ? { ease: a.ease } : {}) })),
    { ease: "linear", precision: 4 },
  );
  const escaped = rebalanceAdditiveChains(raw).replace(/,/g, "\\,");
...
export function escapeDrawtextLiteral(s: string): string {
...
export function buildTipFlashGate(
  tipTimes: number[],
  durSec: number = FLASH_SEC,
): FlashGate | undefined {
  if (tipTimes.length === 0) return undefined;
  const terms = tipTimes.map(
    (b) => `between(t,${fmt3(round3(b))},${fmt3(round3(b + durSec))})`,
  );
  return {
    enable: rebalanceAdditiveChains(terms.join("+")),
    window: {
 ...
 */
export function evalAnchors(anchors: AmountAnchor[], t: number): number {
...
function applyEase(ease: EaseName, u: number): number {
  switch (ease) {
    case "smoothstep":
    case "easeInOut":
      return u * u * (3 - 2 * u);
...
function numOr(v: number | undefined, def: number): number {
