...
describe("buildDualSubLayout", () => {
  ...
  it("bar: canvas extends below the video; the band lives in the bar", () => {
    const l = buildDualSubLayout({ layout: "bar", videoW: 1920, videoH: 1080, sizeProfile: "tv", slots: BOTH });
  ...
  it("solo target hugs the bottom: band tightens when native is absent", () => {
    const solo = buildDualSubLayout({ layout: "stack", videoW: 1920, videoH: 1080, sizeProfile: "tv", slots: { target: true, native: false } });
    const both = buildDualSubLayout({ layout: "stack", videoW: 1920, videoH: 1080, sizeProfile: "tv", slots: BOTH });
  ...
  it("slot edges snap to the 4px grid (GCD-collapsible)", () => {
    const l = buildDualSubLayout({ layout: "stack", videoW: 1024, videoH: 576, sizeProfile: "tv", slots: BOTH });
    for (const id of ["target", "native"] as const) {
  ...
  it("hidden slots are not carved; raw pass-through is a bare F", () => {
    ...
    const raw = buildDualSubLayout({ layout: "stack", videoW: 1920, videoH: 1080, sizeProfile: "tv", slots: { target: false, native: false } });
    expect(raw.m0).toBe("F");
    expect(raw.slotOrder).toEqual(["video"]);
  });

  it("is deterministic", () => {
    const a = buildDualSubLayout({ layout: "stack", videoW: 1280, videoH: 720, sizeProfile: "desktop", slots: BOTH });
    const b = buildDualSubLayout({ layout: "stack", videoW: 1280, videoH: 720, sizeProfile: "desktop", slots: BOTH });
    expect(a).toEqual(b);
  });
});
