...
import { ICON_MAX_EM, computeLayout, SNAP_PX, type PxRect } from "./layout";
import { cardSource, labelChromeSource, magnifierSource, wordStripSources } from "./glyphs";
import { buildCaretBoxes, buildCoverBoxes, buildUnderlineBoxes } from "./boxes";
 ...
 *  clips its content. */
function snapOut(rect: PxRect, canvasW: number, canvasH: number): PxRect {
  const x0 = Math.max(0, Math.floor(rect.x / SNAP_PX) * SNAP_PX);
  const y0 = Math.max(0, Math.floor(rect.y / SNAP_PX) * SNAP_PX);
  const x1 = Math.min(canvasW, Math.ceil((rect.x + rect.w) / SNAP_PX) * SNAP_PX);
  const y1 = Math.min(canvasH, Math.ceil((rect.y + rect.h) / SNAP_PX) * SNAP_PX);
  return { x: x0, y: y0, w: x1 - x0, h: y1 - y0 };
}

/** Translate canvas-absolute gated boxes into a zone cell's local space. */
function localBoxes(boxes: GatedBox[], zone: PxRect): GatedBox[] {
  return boxes.map((box) => ({ ...box, x: box.x - zone.x, y: box.y - zone.y }));
}

/** Tight integer bbox of a box set, padded 1px and clamped to `within`. */
function boxesBBox(boxes: GatedBox[], within: PxRect): PxRect {
  let x0 = Infinity;
  let y0 = Infinity;
  let x1 = -Infinity;
  let y1 = -Infinity;
...
export const SearchTyping: MosaicTemplate<SearchTypingProps> = {
  id: asTemplateId(SEARCH_TYPING_ID),
  label: "Search Typing",
  version: 1,
  description:
  ...
  outputHints: {
    ...
    posterTimeMs: 17200,
    note:
      'A pinned output duration is fit exactly — typing speeds up or stretches uniformly (the hint matches the default words\' natural cadence). endBehavior "hold" keeps the last word typed for the poster; "loop" is seamless.',
  },
  propsSchema: searchTypingPropsSchema,
  defaultProps,
  ...
  async render(
    ...
    try {
      const resolved = parseProps(props);
      const timeline = buildTypingTimeline(
        resolved.words,
        resolved.timing,
        resolved.endBehavior,
        {
          ...
          pinnedDurationMs: resolvePinnedDurationMs(ctx),
          fps: ctx.target.fps,
        },
      );
      ...
      // Real-geometry pieces (Recipe 1): every cell snaps OUTWARD to the
      // SNAP_PX grid; sources are authored local to their SNAPPED cell so
      // nothing rescales and nothing clips. Importance sets paint z-order
      // (card container at the bottom).
      type Piece = { rect: PxRect & { importance: number }; source: MosaicSource };
      const pieces: Piece[] = [];
      ...
      pieces.push({
        ...
        source: cardSource(layout, resolved.style.cardColor),
      });
      if (layout.icon) {
        ...
        pieces.push({
          ...
          source: {
            ...magnifierSource(layout.icon, iconCell, resolved.style.labelColor),
      ...
      const labelRect: PxRect | null =
        resolved.underline === "static" && layout.labelZone
          ? {
              ...layout.labelZone,
      ...
      if (labelRect) {
        ...
        const labelSource = labelChromeSource(layout, labelCell, resolved.style.labelColor, {
      ...
      for (const strip of wordStripSources(layout, wordCell, resolved.style.inkColor, windows)) {
      ...
      pieces.push({
        ...
        source: gatedBoxTrackSource(localBoxes(buildCoverBoxes(timeline, layout), wordCell), {
      ...
      if (resolved.underline === "grow") {
        ...
        pieces.push({
          ...
          source: gatedBoxTrackSource(
            localBoxes(buildUnderlineBoxes(timeline, layout), underlineCell),
            { color: resolved.style.inkColor },
          ),
        });
      }
      if (resolved.caret.show) {
        const caretBoxes = buildCaretBoxes(timeline, layout, resolved.caret);
        const caretCell = snapOut(boxesBBox(caretBoxes, layout.bar), width, height);
        pieces.push({
          rect: { ...caretCell, importance: 4 },
          source: gatedBoxTrackSource(localBoxes(caretBoxes, caretCell), {
            color: resolved.style.inkColor,
          }),
        });
      }
      ...
      for (const layer of placed.layers) {
        const ordered = [...layer.rectIndices].sort(
          (a, b) =>
            pieces[a].rect.y - pieces[b].rect.y || pieces[a].rect.x - pieces[b].rect.x,
        );
        for (const index of ordered) sources.push(pieces[index].source);
      }
      ...
      const iconCapPx = ICON_MAX_EM * layout.fontSize + 2 * 8;
      return withLayoutContract(doc, ctx, {
        templateId: SEARCH_TYPING_ID,
        constraints: layout.icon
          ...
          : [],
        debug: (props as { debugLayout?: boolean }).debugLayout === true,
      });
    } catch (error) {
