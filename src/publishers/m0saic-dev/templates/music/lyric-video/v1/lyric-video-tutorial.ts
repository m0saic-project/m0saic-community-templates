...
function cardTile(
  color: MosaicColor,
  opts: { radius?: number; stroke?: MosaicColor; atSec?: number } = {},
...
function caption(
  ...
  return onboardingLeaf(
    onboardingTextSource({ text, fontSize: type.micro, color, hAlign, label }),
  );
}

/** A horizontal chip bar (the timing lane in miniature). `null` = silence. */
function laneStrip(
  cells: Array<{ color: MosaicColor; atSec?: number } | null>,
  weights: number[],
): OnboardingComposition {
  const lane = onboardingSplit(
    "col",
    weights,
    cells.map((c) => (c ? cardTile(c.color, { radius: 0.3, atSec: c.atSec }) : null)),
  );
  return onboardingOverlay(
    cardTile(P.inset, { radius: 0.12, stroke: P.border }),
    padded(onboardingSplit("row", [1, 2, 1], [null, lane, null]), 2),
  );
}

// ── Beat visuals ──

function songVisual(type: OnboardingTypeRamp): OnboardingComposition {
  const row = (
    label: string,
    color: MosaicColor,
    at: number,
    ...
    onboardingOverlay(
      cardTile(color, { radius: 0.14, atSec: at }),
      caption(label, type, P.textHi, `song row ${label.slice(0, 10)}`),
    );
  const content = onboardingSplit(
    "row",
    [4, 8, 2, 3, 2, 8, 4],
    [
      null,
...
function lyricsVisual(type: OnboardingTypeRamp): OnboardingComposition {
  const chips: Array<{ color: MosaicColor; atSec?: number } | null> = [
    null,
...
function tapVisual(type: OnboardingTypeRamp): OnboardingComposition {
  const card = onboardingOverlay(
  ...
  const keys = onboardingGuttered(
    "col",
    [
      onboardingOverlay(cardTile(P.raised, { radius: 0.2 }), caption("Space · stamp", type, P.textHi, "tap key space")),
      onboardingOverlay(cardTile(P.raised, { radius: 0.2 }), caption("B · silent beat", type, P.textHi, "tap key b")),
      onboardingOverlay(cardTile(P.raised, { radius: 0.2 }), caption("Backspace · undo", type, P.textHi, "tap key undo")),
    ],
    3,
    2,
  );
  return windowCard(onboardingSplit("row", [11, 1, 3], [card, null, keys]));
}

function correctVisual(type: OnboardingTypeRamp): OnboardingComposition {
  const row = (text: string, color: MosaicColor, at: number, name: string) =>
    onboardingOverlay(cardTile(color, { radius: 0.16, atSec: at }), caption(text, type, P.textHi, name));
  const content = onboardingSplit(
    "row",
    [2, 4, 1, 4, 1, 4, 2, 5, 2],
    [
...
function beatsVisual(type: OnboardingTypeRamp): OnboardingComposition {
  const chips: Array<{ color: MosaicColor; atSec?: number } | null> = [
    { color: P.accentDim, atSec: 0.1 },
...
function beatProgress(index: number, type: OnboardingTypeRamp): OnboardingComposition {
  ...
  const dots = onboardingGuttered(
    "col",
    BEATS.map((_, i) =>
      cardTile(i === index ? P.accent : i < index ? P.accentDim : P.border, { radius: 0.5 }),
    ),
    3,
    2,
  );
...
export function renderLyricVideoTutorial(ctx: MosaicEngineContext): MosaicDocumentPipeline {
