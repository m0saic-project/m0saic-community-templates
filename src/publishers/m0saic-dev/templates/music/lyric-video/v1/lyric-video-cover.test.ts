...
describe("lyric-video cover (mosaic-branding kit)", () => {
  it("builds the branded pane cover across landscape and portrait", () => {
    for (const size of [[1280, 720], [720, 1280]] as const) {
      const doc = renderLyricVideoCover(ctx(size[0], size[1]));
      ...
      expect(parseM0StringToRenderFrames(doc.m0, size[0], size[1])).toHaveLength(doc.sources.length);
      const labels = doc.sources.map((s) => s.editor?.label ?? "");
      // The family chat pane + this template's real-material hero.
      const has = (prefix: string) => labels.some((l) => l.startsWith(prefix));
  ...
  it("carries the brand mark assets and stays deterministic", () => {
    const a = renderLyricVideoCover(ctx(1280, 720));
    const b = renderLyricVideoCover(ctx(1280, 720));
...
describe("lyric-video tutorial (SaaS beats)", () => {
  it("is a five-beat pipeline of validated pages with authored durations", () => {
    const pipeline = renderLyricVideoTutorial(ctx(1280, 720));
    ...
    for (const step of pipeline.steps) {
      const doc = step.file as { m0: string; sources: Array<{ editor?: { label?: string } }> };
      expect(isValidM0String(doc.m0)).toBe(true);
      expect(parseM0StringToRenderFrames(doc.m0, 1280, 720)).toHaveLength(doc.sources.length);
  ...
  it("every beat carries the progress rail", () => {
    const pipeline = renderLyricVideoTutorial(ctx(1280, 720));
    for (const step of pipeline.steps) {
      const doc = step.file as { sources: Array<{ editor?: { label?: string } }> };
